    ...
    >
      ...
      >
        <omsp-panel>
          ...
          <div v-if="item.title === '事件分类'" style="margin: 15px">
            <!--title-->
          ...
          <div v-if="item.title === '达成情况'" style="margin: 15px">
            ...
            <div style="width: 100%;display: flex;align-items: center;height: 30px;">
              ...
              <div style="flex: 1.5;text-align: right;display: flex;justify-content: flex-end;align-items: center">
                ...
                <div
                  style="margin-left: 9px;width: 20px;height: 5px ;border-radius: 0px 50px 50px 0px;background-image: linear-gradient(to right, #F5AC18 , #F3C871);"
                />
              </div>
            </div>
            <!--body-->
            ...
            >
              ...
              <div style="display: flex;">
                <div style="width: 80%">
                  <div
                    :style="{'width':(itemAchieve.achieveValue/(achieveMax===0?1:achieveMax)).toFixed(2)*100+'%','background-color':'red','height':'16px','margin-top':'10px','border-radius':'0px 50px 50px 0px','background-image': 'linear-gradient(to right, #075AE4 , #249DEB)'}"
                  />
                </div>
              ...
              <div style="display: flex;margin-bottom: 20px">
                <div style="width: 80%">
                  <div
                    :style="{'width':(itemAchieve.noAchieveValue/(achieveMax===0?1:achieveMax)).toFixed(2)*100+'%','background-color':'red','height':'16px','margin-top':'10px','border-radius':'0px 50px 50px 0px','background-image': 'linear-gradient(to right, #F5AC18 , #F3C871)'}"
                  />
                </div>
                <div style="width: 20%;margin-top: 8px;text-align: right">
...
export default {
  ...
  computed: {
    ...
    achieveMax() {
      const result = Math.max(
        ...[
          /* 达成情况最大值*/
          Math.max(...this.tab[2].data.achieved.map(item => {
            return item.value
          })),
          /* 未达成情况最大值*/
          Math.max(...this.tab[2].data.noAchieved.map(item => {
            return item.value
          }))
        ]
      )
  ...
  methods: {
    ...
    getOption(tabNum) {
      ...
      } else if (this.tab[tabNum].title === '满意度') {
